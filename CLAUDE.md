...
- **테스트**: vitest (프론트) + pytest (RPi 펌웨어 `rpi_firmware/__tests__/`)
...
- **ROS 2** (`ros2_ws/src/waste_robot/`, 패키지 골격 작성됨 — Humble + Nav2 + webots_ros2): fsm/navigation/ekf/qr/mqtt 등 16개 노드 + `full_system.launch.py` + `nav2_params.yaml`/`ekf_params.yaml`. 아직 실기 검증 전
...
python -m tools.web_control                        # 폰 브라우저 수동 조종 + 카메라 MJPEG + YOLO 검출 + 시계열 모터로그/CSV (http://<RPi>:8080)
...
2. **TCP extern 경로** — ROS 2 이식용 (`webots_sim/README.md` 참조)
   ...
   - ROS 2 패키지는 `ros2_ws/src/waste_robot/`에 **골격 작성 완료** (위 "기술 스택" 참조) — Webots extern 연결 + 실기 검증은 미완
   - 현재 Webots 컨트롤러: `Robot_controller/`, `Prototype_controller/`, `Obstacle_sync_controller/`, `Patrol_controller/`
...
**펌웨어 인지(perception) 모듈** (planner가 주입받아 사용):
- `perception.py` — QR bbox 중심 x좌표 → 카메라 HFOV 매핑으로 빈 bearing 추정, 거리는 전방 초음파 우선. target QR id 매칭 시에만 `locked=True`.
- `human_guard.py` — 후방 카메라(YOLO) + 측면/후방 초음파 융합으로 주변 장애물 판단. `config.PERSON_WAIT_S` 이상 블록 지속 시 우회 트리거.
- 테스트: `pytest rpi_firmware/__tests__/` (현재 `test_perception.py`).
